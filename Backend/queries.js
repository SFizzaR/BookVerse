...
async function insertBooksIntoDatabase(books) {
    ...
    try {
        ...
        for (let book of books) {
            ...
            // 1. Insert author into the Authors table (if not already present)
            let authorExistsQuery = `SELECT author_id FROM Authors WHERE author_id = :author_id`;
            const authorExists = await connection.execute(authorExistsQuery, { author_id: authorId });
            ...
            // 2. Insert the book into the Books table, using the author_id as a reference
            await connection.execute(
                `INSERT INTO Books(book_title, genre, author_id, publish_date, ratings) 
                 VALUES (:book_title, :genre, :author_id, :publish_date, :ratings)`,
                { 
                    book_title: bookTitle, 
                    genre: genre,
                    author_id: authorId,  // Now referencing author_id from Authors table
