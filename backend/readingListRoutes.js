...
// Route to update status in the reading list
router.post('/updateStatus', verifyToken, async (req, res) => {
    const { user_id, username, book_id, book_title, author_id, author_name, status } = req.body;
    let connection;
    ...
    try {
        ...
        await connection.execute(
            ...
            `,
            { user_id, username, book_id, book_title, author_id, author_name, status }
        );

        res.status(200).json({ message: 'Reading list updated successfully' });
    } catch (error) {
        console.error('Error updating reading list:', error);
        res.status(500).json({ message: 'An error occurred while updating the reading list' });
    } finally {
        if (connection) {
